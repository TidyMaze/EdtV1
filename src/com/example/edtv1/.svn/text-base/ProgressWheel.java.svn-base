...
public class ProgressWheel extends View {
    ...
    private static final float ATURN = 360;
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        //Draw the rim
        canvas.drawArc(circleBounds, ATURN, ATURN, false, rimPaint);
        canvas.drawArc(circleOuterContour, ATURN, ATURN, false, contourPaint);
        canvas.drawArc(circleInnerContour, ATURN, ATURN, false, contourPaint);
        //Draw the bar
        if (isSpinning) {
    ...
    */
    ...
    /**
     * Increment the progress by 1 (of ATURN)
     */
    public void incrementProgress() {
        ...
        if (progress > ATURN)
            progress = 0;
		if (drawCount) {
			setText(Math.round(((float) progress / ATURN) * 100) + "%");
		}
        spinHandler.sendEmptyMessage(0);
    }
